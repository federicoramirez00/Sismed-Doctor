...
class Disponibilidad extends Validator
{
	...
	{
		$sql = 'SELECT id_disponibilidad, id_dia, hora_inicio, hora_fin FROM disponibilidad WHERE id_disponibilidad = ?';
	...
	{
		$sql = 'UPDATE disponibilidad SET id_dia = ?, hora_inicio = ?, hora_fin = ? WHERE id_disponibilidad = ?';
		$params = array($this->dia, $this->horainicio, $this->horafin, $this->id);
		return Database::executeRow($sql, $params);
	}
